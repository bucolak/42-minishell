...
/*   Updated: 2025/08/24 17:21:45 by bucolak          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	apply_parser(char *line, t_general *pipe_blocs, t_env *env, t_full *full)
{
	add_history(line);
...
void end_loop_main(t_now *get, t_general **pipe_blocs, char *line, int *last_dqm)
{
	if(get)
		free_envp(get);
...
// void main_loop_frst(t_general **pipe_blocs, int last_dqm, t_now *get, t_env *env)
// {
// 	if (signal_ec == 1)
// 	{
// 		last_dqm = 130;
...
int	main(int argc, char *argv[], char **envp)
{
	...
	while (1)
	{
		...
		end_loop_main(get, &pipe_blocs, line, &last_dqm);
	}
	free_env(env);
	free_envp(get);
	free_pipe_blocks(pipe_blocs);
